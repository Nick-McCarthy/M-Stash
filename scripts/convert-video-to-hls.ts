 ...
 * 
 * Usage:
 *   tsx scripts/convert-video-to-hls.ts <input-video> [output-directory]
 * 
 * Example:
 *   tsx scripts/convert-video-to-hls.ts movie.mp4
 *   tsx scripts/convert-video-to-hls.ts movie.mp4 output-folder
 */
...
function getVideoInfo(videoPath: string): VideoInfo {
  try {
    // Use ffmpeg to probe video and get info in JSON format
    // Note: ffmpeg-static includes ffprobe, so we can use it directly
    ...
    // Fallback: try to get info from stderr output
    try {
...
function createMasterPlaylist(
  ...
  validResolutions.forEach((res) => {
    ...
    const resolution = `${res.height * 16 / 9}x${res.height}`; // Assuming 16:9, adjust if needed
...
function main() {
  ...
  if (args.length === 0) {
    console.error("Usage: tsx scripts/convert-video-to-hls.ts <input-video> [output-directory]");
    process.exit(1);
  }
  ...
  if (!ffmpegPath) {
    console.error("Error: ffmpeg-static not found. Please install it:");
